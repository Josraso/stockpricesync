...
if (Tools::isSubmit('process_queue')) {
    ...
    if ($result['success']) {
        ...
        // Assign shop name for JavaScript in the template
        ...
        if ($shop_configured) {
            if ($shop_type == 'MAIN') {
                ...
                // View specific shop details or edit shop form
                if ((Tools::isSubmit('view_shop') || Tools::isSubmit('edit_shop')) && Tools::isSubmit('id_shop_remote')) {
                    $id_shop_remote = (int)Tools::getValue('id_shop_remote');
                    $shop = new SPSRemoteShop($id_shop_remote);
                    
                    if (Validate::isLoadedObject($shop)) {
                        // Asignar la tienda directamente a Smarty para evitar problemas
                        $this->context->smarty->assign('view_shop', $shop);
                        
                        if (Tools::isSubmit('edit_shop')) {
                            $this->context->smarty->assign('edit_shop', $shop);
                        } else {
                            ...
                            $this->context->smarty->assign('shop_stats', $stats);
                        }
                    }
                }
                // Shop list view
        ...
        // Assign base smarty variables
        foreach ($view_data as $key => $value) {
            // No sobrescribir variables ya asignadas directamente
            if (!in_array($key, ['view_shop', 'edit_shop', 'shop_stats'])) {
        ...
        if (!$shop_configured) {
            ...
            elseif (Tools::isSubmit('view_logs')) {
                // Logs view - procesamiento de filtros
                ...
                // Process filters
                    ...
                    if (Tools::isSubmit('filter_logs') || Tools::getValue('date_from') || Tools::getValue('date_to') || 
                        Tools::getValue('sync_type') || Tools::getValue('status') !== '' || Tools::getValue('shop_id') || 
                        ...
                        if ($shop_id = Tools::getValue('shop_id')) {
                            $filters['shop_id'] = (int)$shop_id;
                            $view_data['filter_shop_id'] = (int)$shop_id;
                            $has_active_filters = true;
                        }
   ...
   /**
    ...
    {
        $id_shop_remote = (int)$id_shop_remote;
        if (!$id_shop_remote) {
            return 0;
        }
        
        $sql = 'SELECT COUNT(*) FROM `'._DB_PREFIX_.'stockpricesync_log` 
                WHERE id_shop_remote = '.$id_shop_remote;
    ...
    private function getShopSuccessRate($id_shop_remote)
    {
        ...
        $success = (int)Db::getInstance()->getValue('
            ...
            WHERE id_shop_remote = '.$id_shop_remote.' AND status = 1
        ');
    ...
    {
        // Verificar que id_shop_remote es un entero positivo
        $id_shop_remote = (int)$id_shop_remote;
        if ($id_shop_remote <= 0) {
            return null;
        }
        
        // Usar una consulta SQL simple sin LIMIT
        $sql = "SELECT date_add FROM `"._DB_PREFIX_."stockpricesync_log` 
                WHERE id_shop_remote = {$id_shop_remote} 
                ORDER BY date_add DESC";
        ...
        if (!empty($result) && isset($result[0]['date_add'])) {
