...
<div class="panel">
    ...
    {else}
        <div class="alert alert-warning">
            <p>{l s='Shop information not found. Please go back to the dashboard and try again.' mod='stockpricesync'}</p>
            <a href="{$current_link}" class="btn btn-default">
                <i class="icon-arrow-left"></i> {l s='Back to Dashboard' mod='stockpricesync'}
            </a>
        </div>
    {/if}
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#test_connection").click(function() {
            ...
            var shopUrl = "{if isset($view_shop)}{$view_shop->url|escape:'javascript':'UTF-8'}{/if}";
            var apiKey = "{if isset($view_shop)}{$view_shop->api_key|escape:'javascript':'UTF-8'}{/if}";
            ...
            $.ajax({
                ...
                data: JSON.stringify({
                    shop_name: "{if isset($shop_name)}{$shop_name|escape:'javascript':'UTF-8'}{/if}",
                    test: true
                }),
                headers: {
                    'X-API-Key': apiKey
